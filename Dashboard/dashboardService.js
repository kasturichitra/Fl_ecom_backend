...
export const getOrdersByPaymentMethod = async (tenantId, filters = {}) => {
  ...
  const { orderModelDB, offlineOrderTransactionsModelDB } = await getTenantModels(tenantId);
  ...
  const onlineStats = await orderModelDB.aggregate([
    { $match: { ...baseQuery, payment_status: "Successful" } },
  ...
  const offlineStats = await offlineOrderTransactionsModelDB.aggregate([
...

    {
      $group: {
        ...
        offline_count: { $sum: 1 },
        offline_value: { $sum: "$amount" },
      },
    },
  ]);

  /* -------------------- MERGE BY PAYMENT METHOD -------------------- */
  const paymentMap = {};
  ...
  // Offline data
  for (const item of offlineStats) {
    const method = item._id;
    ...
    if (!paymentMap[method]) {
      ...
      paymentMap[method].offline_count = item.offline_count;
      paymentMap[method].offline_value = item.offline_value;
    }
  }

  /* -------------------- FINAL RESPONSE -------------------- */
  return {
    OrderPaymentmethod: Object.values(paymentMap),
  };
};
...
export const getOrdersTrendService = async (tenantId, filters = {}) => {
  ...
  const pipelineOnlie = [
    ...
    ...(Object.keys(matchCriteria).length > 0
      ? [{ $match: { ...matchCriteria, payment_status: "Successful" } }]
  ...
  const aggregationResultOnline = await orderModelDB.aggregate(pipelineOnlie);
  const aggregationResultOffline = await offlineOrderModelDB.aggregate(pipelineOffline);
