...
import {
  sendBusinessApprovalEmailToAdmin,
  sendBusinessVerificationSuccessEmail,
...
export const gstinVerifyService = async (payload) => {
  ...
  if (
    data.data.companyName.toLowerCase().trim() ===
    payload.business_name.toLowerCase().trim()
  ) {
    return data;
  } else {
...
export const addBusinessDetailsService = async (
  tenantId,
  user_id,
  businessData,
  files
  ...
  const existingBusiness = await businessModelDB.findOne({
...
export const getAllBusinessDetailsService = async (
  tenantId,
  { assigned_to, page = 1, limit = 10, sort }
) => {
  throwIfTrue(!tenantId, "Tenant ID is Required");
...
export const assignBusinessDetailsService = async (
  tenantId,
  business_unique_id,
  updateData
) => {
  ...
  if (updateData.is_verified === true) {
    ...
    if (user && user.email) {
      await sendBusinessVerificationSuccessEmail(
        user.email,
...
export const deactivateBusinessService = async (
  tenantId,
  user_id,
  gstinNumber
) => {
...
const backgroundEmailProcess = async (tenantId, businessData) => {
  try {
    ...
    const admins = await userModelDB.find(
