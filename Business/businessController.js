...
export const addBusinessDetailsController = async (req, res) => {
  try {
    ...
    if (!validation.isValid) {
      return res
        .status(400)
        .json(errorResponse(validation.message, validation.errors));
    }

    const response = await addBusinessDetailsService(
      tenantId,
      user_id,
      req.body,
      req.files
    ...
    res
      .status(200)
      .json(successResponse(response.message, { data: response.user }));
  } catch (error) {
    res.status(500).json(errorResponse(error.message, error));
  }
};
...
export const getBusinessDetailsController = async (req, res) => {
  try {
    ...
    const response = await getBusinessDetailsService(
      tenantId,
      business_unique_id
    );
    res
...
export const getAllBusinessDetailsController = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(successResponse("Business details fetched successfully", response));
  } catch (error) {
    res.status(500).json(errorResponse(error.message, error));
  }
};
...
export const assignBusinessDetailsController = async (req, res) => {
  try {
    const tenantId = req.headers["x-tenant-id"];
    const { id: business_unique_id } = req.params;
    const response = await assignBusinessDetailsService(
      tenantId,
      business_unique_id,
      req.body
    );
...
export const deactivateBusinessController = async (req, res) => {
  try {
    ...
    const response = await deactivateBusinessService(
      tenantId,
      user_id,
      getinumber
    );
    res
      .status(200)
      .json(
        successResponse("Business deactivated successfully", { data: response })
      );
  } catch (error) {
    res.status(500).json(errorResponse(error.message, error));
  }
