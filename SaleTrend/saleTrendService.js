...

    {
      $facet: {
        data: [
          ...
          /* --------------------------------------------
             🔥 LOOKUP ONLY ACTIVE PRODUCTS
          --------------------------------------------- */
          {
            $lookup: {
              from: ProductModelDB.collection.name,
              let: { productIds: "$trend_products.product_unique_id" },
              pipeline: [
                {
                  $match: {
                    $expr: {
                      $and: [{ $in: ["$product_unique_id", "$$productIds"] }, { $eq: ["$is_active", true] }],
                    },
                  },
                },
                {
                ...
                $slice: [
                  {
                    $filter: {
                      ...
                      as: "item",
                      cond: { $ne: ["$$item", null] }, // remove inactive
                    },
                  },
                  products_limit,
                ],
