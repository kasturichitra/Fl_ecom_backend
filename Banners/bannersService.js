...
export const createBannersServices = async (tenantID, bannerData) => {
    try {
        if (!tenantID) throw new Error("Tenant ID is required");

        const bannerModelDB = await OfferBannerModel(tenantID);
...
export const getAllBannersServices = async (tenantID) => {
    try {
        if (!tenantID) throw new Error("Tenant ID is required");
...
export const updatebannersServices = async (tenantID, bannerId, updatedData, newImageFilename) => {
    try {
        if (!tenantID) throw new Error("Tenant ID is required");
...
export const deleteBannerServices = async (tenantID, bannerId) => {
  try {
    if (!tenantID) throw new Error("Tenant ID is required");
    if (!bannerId) throw new Error("Banner ID is required");
