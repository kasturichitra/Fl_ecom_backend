...
import OfflineOrderModel  from "../Orders/OfflineOrders/offlineOrderModel.js";
...
import { BusinessModel } from "../Business/businessModel.js";
...
import PaymentTransactionsModel from "../PaymentTransactions/paymentTransactionModel.js";
import BannerModel from "../Banners/bannerModel.js";
import EcomFeatureModel from "../EcomFeatures/ecomFeatureModel.js";
import ThemeModel from "../ThemeConfig/themeModel.js";
import OfflineOrderTransactionsModel from "../OfflineOrderTransactions/offlineOrderTransactionsModel.js";
import OfflineCustomerModel from "../OfflineCustomere/OfflineCustomerModel.js";
...
export const getTenantModels = (tenantId) => {
  if (!tenantModelPromises.has(tenantId)) {
    const promise = (async () => {
      ...
      const offlineOrderModelDB = await OfflineOrderModel(tenantId);
      ...
      const businessModelDB = await BusinessModel(tenantId);
      ...
      const paymentTransactionsModelDB = await PaymentTransactionsModel(tenantId);
      const bannerModelDB = await BannerModel(tenantId);
      const ecomFeatureModelDB = await EcomFeatureModel(tenantId);
      const themeModelDB = await ThemeModel(tenantId);
      const offlineOrderTransactionsModelDB = await OfflineOrderTransactionsModel(tenantId);
      const offlineCustomerModelDB = await OfflineCustomerModel(tenantId);
